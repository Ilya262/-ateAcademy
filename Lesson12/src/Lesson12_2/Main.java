...
import Lesson12_2.Months;
import Lesson12_2.Seasons;

public class Main {

	public static void main(String[] args) throws WrongInputConsoleParametersException {

		List<Seasons> seasons_mass = new ArrayList<Seasons>();// array "Seasons"
		List<Months> months_mass = new ArrayList<Months>();// array "Months"
	for(Months Months:Months.values()) {
		months_mass.add(Months);
	}
	for(Seasons Seasons:Seasons.values()) {
		seasons_mass.add(Seasons);
	}
		while (true) {
			...
			case 6: {// Next Seasons
				...
				if (flag) {
					...
					if (ordinal == seasons_mass.size() - 1) {
						ordinal = 0;
						System.out.println(seasons_mass.get(ordinal));
					} else {
						System.out.println(seasons_mass.get(ordinal+ 1));
					}
					if (!flag) {
						String mess = ("Season is not exist");
						throw new WrongInputConsoleParametersException(mess);
	...
	private static boolean seasons_exist(List<Seasons> seasons_mass, String seasonSc) {// method exist seasons
		boolean flag = false;
	...
	private static boolean months_exist(List<Months> months_mass, String m) {// method exist months
		boolean flag = false;
